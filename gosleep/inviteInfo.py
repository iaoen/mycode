...
def invite_info(key, iv, device_id, authorization, uid):
    ...
    res = json.loads(aes.decrypt(res_content))
    reward_subordinate_count = res["data"]["reward_subordinate_count"]
    subordinate_count = res["data"]["subordinate_count"]
    invitee_code = res["data"]["invitee_code"]
    print("invite_info", uid, invitee_code, reward_subordinate_count, "/", subordinate_count)
...
def main(i):
    ...
    invite_info(key, iv, device_id, authorization, uid)
...
for i in ranges:
    print(f"------------------------------{i+1}------")
    tt_start = str(int(time.mktime(time.strptime(f"2023-4-{datetime.datetime.today().day-1} 22:{random.randint(10,59)}:{random.randint(10,59)}", "%Y-%m-%d %H:%M:%S"))))
    tt_stop = str(int(time.mktime(time.strptime(f"2023-4-{datetime.datetime.today().day} 08:{random.randint(10,59)}:{random.randint(10,59)}", "%Y-%m-%d %H:%M:%S"))))
