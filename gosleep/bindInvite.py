...
zh_list2 = open("./gosleep/zh2.txt").read().split("\n")
zh_list3 = open("./gosleep/zh3.txt").read().split("\n")
ranges = range(27 - 1, 99999)
for i in ranges:
    print(f"------------------------------{i+1}------")
    _, _, invite, _, _, _, _ = zh_list2[i].split("|")
    for ii in range((i*3),(i*3)+3):
        email, uid, _, key_b64, iv_b64, device_id, authorization = zh_list3[ii].split("|")
        key, iv = base64.b64decode(key_b64).hex(), base64.b64decode(iv_b64).hex()
        bind_invite_code(key, iv, device_id, authorization, uid, invite)
